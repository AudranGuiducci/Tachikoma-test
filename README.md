...
* [`src/tashikoma_form/`](src/tashikoma_form/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/tashikoma_form/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `tashikoma-form` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tashikoma_form/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tashikoma-form.example
  (:require [tashikoma-form.config :as config])
